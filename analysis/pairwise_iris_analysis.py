...
def precompute_codes(images, classifier, rotation):
    ...
    for index, image in enumerate(images, start=1):
        ...
        if iris_band is None or iris_mask is None:
            raise RuntimeError(f"Segmentation failed for sample index {index - 1}")
...
def main():
    ...
    parser.add_argument(
        "--output",
        default=str(DEFAULT_OUTPUT_DIR / "pairwise_scores.npz"),
        help="Output .npz file for pairwise scores and metrics",
    )
    parser.add_argument(
        "--figure-output",
        default=str(DEFAULT_OUTPUT_DIR / "pairwise_analysis.png"),
        help="Output image path for the distribution/ROC figure",
    )
    parser.add_argument(
        "--rotation",
        type=int,
    ...
    plot_results(pairwise["scores"], pairwise["same_class"], evaluation, figure_path=args.figure_output)
